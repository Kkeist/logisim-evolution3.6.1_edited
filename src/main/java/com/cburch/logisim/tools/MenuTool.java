...
public class MenuTool extends Tool {
  ...
  private static class MenuComponent extends JPopupMenu implements ActionListener {
    ...
    final JMenuItem deleteEntireWire = new JMenuItem("Delete entire wire");
    final JMenuItem selectEntireWire = new JMenuItem("Select entire wire");

    MenuComponent(Project proj, Circuit circ, Component comp, Location click) {
      this.proj = proj;
      this.circ = circ;
      this.comp = comp;
      this.click = click;
      ...
      // Delete / Select entire wire when right-click on pin
      if (click != null && isComponentEndAt(comp, click)) {
        Collection<Wire> wiresAtPin = circ.getWires(click);
        if (!wiresAtPin.isEmpty()) {
          addSeparator();
          add(selectEntireWire);
          selectEntireWire.addActionListener(this);
          if (canChange) {
            add(deleteEntireWire);
            deleteEntireWire.addActionListener(this);
          }
        }
      }
    }
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (src == del) {
        ...
        proj.doAction(
            xn.toAction(S.getter("rotateComponentAction", comp.getFactory().getDisplayGetter())));
      } else if (src == selectEntireWire) {
        Selection sel = proj.getSelection();
        if (sel != null) {
          Collection<Wire> wiresAtPin = circ.getWires(click);
          if (!wiresAtPin.isEmpty()) {
            WireSet wireSet = circ.getWireSet(wiresAtPin.iterator().next());
            Set<Wire> toSelect = new HashSet<>();
            for (Wire w : circ.getWires()) {
              if (wireSet.containsWire(w)) toSelect.add(w);
            }
            if (!toSelect.isEmpty()) sel.setSelected(toSelect);
          }
        }
      } else if (src == deleteEntireWire) {
        final var circ = proj.getCurrentCircuit();
        Collection<Wire> wiresAtPin = circ.getWires(click);
        if (!wiresAtPin.isEmpty()) {
          Wire firstWire = wiresAtPin.iterator().next();
          WireSet wireSet = circ.getWireSet(firstWire);
          Set<Wire> wiresToDelete = new HashSet<>();
          for (Wire w : circ.getWires()) {
            if (wireSet.containsWire(w)) wiresToDelete.add(w);
          }
          if (!wiresToDelete.isEmpty()) {
            final var xn = new CircuitMutation(circ);
            for (Wire w : wiresToDelete) xn.remove(w);
            proj.doAction(
                xn.toAction(
                    new com.cburch.logisim.util.StringGetter() {
                      @Override
                      public String toString() {
                        return "Delete entire wire";
                      }
                    }));
          }
        }
  ...
  private static class MenuWire extends JPopupMenu implements ActionListener {
    private static final long serialVersionUID = 1L;
    final Project proj;
    final Circuit circ;
    final Wire wire;
    ...
    MenuWire(Project proj, Circuit circ, Wire wire) {
      this.proj = proj;
      this.circ = circ;
      this.wire = wire;
      boolean canChange = proj.getLogisimFile().contains(circ);
    ...
    public void actionPerformed(ActionEvent e) {
      Object src = e.getSource();
      if (src == selectEntireWire) {
        Selection sel = proj.getSelection();
        if (sel != null) {
          WireSet wireSet = circ.getWireSet(wire);
          Set<Wire> toSelect = new HashSet<>();
          for (Wire w : circ.getWires()) {
            if (wireSet.containsWire(w)) toSelect.add(w);
          }
  ...
  public void mousePressed(Canvas canvas, Graphics g, MouseEvent e) {
    ...
    if (!in_sel.isEmpty()) {
      ...
      if (sel.getComponents().size() > 1) {
        ...
        menu = new MenuComponent(proj, canvas.getCircuit(), comp, pt);
      ...
      if (!cl.isEmpty()) {
        ...
        if (comp instanceof Wire) {
          menu = new MenuWire(proj, canvas.getCircuit(), (Wire) comp);
        } else {
          menu = new MenuComponent(proj, canvas.getCircuit(), comp, pt);
          final var extender = (MenuExtender) comp.getFeature(MenuExtender.class);
          if (extender != null) extender.configureMenu(menu, proj);
  ...
  private static boolean isComponentEndAt(Component comp, Location loc) {
    for (final var end : comp.getEnds()) {
      if (loc.equals(end.getLocation())) return true;
    }
    return false;
  }
