...
public class Project {
  ...
  public Project(LogisimFile file) {
    ...
    addProjectListener(
        e -> {
          if (e.getAction() != ProjectEvent.ACTION_COMPLETE) return;
          if (!AppPreferences.AUTO_SAVE_ENABLED.get()) return;
          if (!AppPreferences.AUTO_SAVE_MODE_ON_ACTION.equals(AppPreferences.AUTO_SAVE_MODE.get())) return;
          Project proj = e.getProject();
          Action last = proj.getLastAction();
          if (last == null || !last.isModification()) return;
          SwingUtilities.invokeLater(() -> ProjectActions.tryAutoSave(proj));
        });
    setLogisimFile(file);
  }
  ...
  public void setFrame(Frame value) {
    ...
    if (value != null) {
      ...
      if (AppPreferences.AUTO_SAVE_ENABLED.get()
          && AppPreferences.AUTO_SAVE_MODE_INTERVAL.equals(AppPreferences.AUTO_SAVE_MODE.get())) {
        int sec = Math.max(1, AppPreferences.AUTO_SAVE_INTERVAL_SECONDS.get());
        autoSaveTimer =
            new Timer(
                sec * 1000,
                ev -> {
                  if (frame != null) ProjectActions.tryAutoSave(this);
                });
