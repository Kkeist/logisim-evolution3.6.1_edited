...
import com.cburch.logisim.circuit.BeautifyWiresTransaction;
...
public class SelectionActions {
  ...
  /** Beautifies wire layout (orthogonalize diagonals, then merge/split). Uses selection if non-empty. */
  public static Action beautifyWires(Circuit circuit, Selection sel) {
    return new BeautifyWires(circuit, sel);
  }
  ...
  private static class BeautifyWires extends Action {
    private final Circuit circuit;
    private final java.util.Collection<Wire> wiresToBeautify;
    private final java.util.Collection<Component> componentsToMove;
    private CircuitTransaction xnReverse;

    BeautifyWires(Circuit circuit, Selection sel) {
      this.circuit = circuit;
      if (sel != null && !sel.isEmpty()) {
        java.util.ArrayList<Wire> wires = new java.util.ArrayList<>();
        java.util.ArrayList<Component> comps = new java.util.ArrayList<>();
        ...
        this.wiresToBeautify = wires.isEmpty() ? null : wires;
        this.componentsToMove = comps; // empty = selection active but no components → move none
      } else {
        this.wiresToBeautify = null;
        this.componentsToMove = null;   // null = no selection → move all components
      }
    }

    @Override
    public void doIt(Project proj) {
      CircuitTransaction xn = new BeautifyWiresTransaction(circuit, wiresToBeautify, componentsToMove);
      CircuitTransactionResult result = xn.execute();
      xnReverse = result.getReverseTransaction();
    }
    ...
    @Override
    public String getName() {
      return S.get("beautifyWiresAction");
    }
