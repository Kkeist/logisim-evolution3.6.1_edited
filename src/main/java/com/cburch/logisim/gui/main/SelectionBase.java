...
class SelectionBase {
  ...
  /** Replace selection with given components (clears lifted). For use when selecting a wire net. */
  public void setSelected(Collection<? extends Component> comps) {
    selected.clear();
    lifted.clear();
    shouldSnap = false;
    bounds = Bounds.EMPTY_BOUNDS;
    if (comps != null && !comps.isEmpty()) selected.addAll(comps);
    fireSelectionChanged();
  }
  ...
  private HashMap<Component, Component> copyComponents(
    ...
    for (Component comp : components) {
      ...
      if (!translate && attrs.containsAttribute(StdAttr.LABEL) && usedLabels != null) {
        String oldLabel = attrs.getValue(StdAttr.LABEL);
        if (oldLabel != null && !oldLabel.isEmpty()) {
          String newLabel =
              getUniqueLabelForCopy(circuit, oldLabel, comp.getFactory(), usedLabels);
  ...
  /**
   * Returns a label that is unique in the circuit when pasting/duplicating. If the original label
   * is already used, appends or increments a number (e.g. SW1 -> SW2).
   */
  private static String getUniqueLabelForCopy(
      Circuit circuit,
      String label,
    ...
    if (!usedLabels.contains(label.toLowerCase())
        && Circuit.IsCorrectLabel(
            circuit.getName(), label, circuit.getNonWires(), null, factory, false)) {
      return label;
    }
    String base;
    int startIndex;
    if (label.length() > 0 && Character.isDigit(label.charAt(label.length() - 1))) {
      int i = label.length() - 1;
      while (i > 0 && Character.isDigit(label.charAt(i - 1))) i--;
      base = label.substring(0, i);
      try {
        startIndex = Integer.parseInt(label.substring(i));
    ...
    for (int n = startIndex; ; n++) {
      ...
      if (usedLabels.contains(candidate.toLowerCase())) continue;
      if (!Circuit.IsCorrectLabel(
          circuit.getName(), candidate, circuit.getNonWires(), null, factory, false)) {
        continue;
      }
      return candidate;
    }
