...
class LayoutOptions extends OptionsPanel {
  ...
  private final PrefOptionList autoSaveMode;
  ...
  public LayoutOptions(PreferencesFrame window) {
    ...
    checks =
        new PrefBoolean[] {
          ...
          new PrefBoolean(AppPreferences.AUTO_SAVE_ENABLED, S.getter("layoutAutoSaveEnabled")),
        };
    autoSaveMode =
        new PrefOptionList(
            AppPreferences.AUTO_SAVE_MODE,
            S.getter("layoutAutoSaveMode"),
            new PrefOption[] {
              new PrefOption(
                  AppPreferences.AUTO_SAVE_MODE_ON_ACTION, S.getter("layoutAutoSaveModeOnAction")),
              new PrefOption(
                  AppPreferences.AUTO_SAVE_MODE_INTERVAL, S.getter("layoutAutoSaveModeInterval"))
            });
    int intervalSec = AppPreferences.AUTO_SAVE_INTERVAL_SECONDS.get();
    autoSaveIntervalSpinner =
        new JSpinner(new SpinnerNumberModel(Math.max(10, Math.min(600, intervalSec)), 10, 600, 10));
    autoSaveIntervalSpinner.addChangeListener(
        e ->
            AppPreferences.AUTO_SAVE_INTERVAL_SECONDS.set(
                (Integer) autoSaveIntervalSpinner.getValue()));
    ...
    panel.add(autoSaveMode.getJLabel());
    panel.add(autoSaveMode.getJComboBox());
    panel.add(new JLabel(S.get("layoutAutoSaveIntervalLabel")));
    panel.add(autoSaveIntervalSpinner);
