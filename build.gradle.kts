...
/**
 * Runs a command and returns stdout on success, or null on failure (e.g. not a git repo).
 */
fun runCommandOrNull(params: List<String>): String? {
  return try {
    val procBuilder = ProcessBuilder()
    procBuilder
      .redirectOutput(ProcessBuilder.Redirect.PIPE)
    ...
    val proc = procBuilder.start()
    val rc = proc.waitFor()
    if (rc == 0) proc.inputStream.bufferedReader().readText().trim() else null
  } catch (e: Exception) {
    null
  }
}
 ...
 * Works even when not inside a git repository (uses fallback values).
 */
fun genBuildInfo(buildInfoFilePath: String) {
  val now = Date()
  val nowIso = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssZ").format(now)
  val branchName = runCommandOrNull(listOf("git", "-C", "$projectDir", "rev-parse", "--abbrev-ref", "HEAD"))
      ?: "unknown"
  val branchLastCommitHash = runCommandOrNull(listOf("git", "-C", "$projectDir", "rev-parse", "--short=8", "HEAD"))
      ?: "unknown"
